...
/*   Updated: 2021/01/11 19:15:53 by dpiedra          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_error(t_all *all, char *str)
{
	...
	while (*str == ' ')
		str++;
	if (*str)
		ft_error(all, 30, "Error : too many color inputs\n");
}

void	ft_ceiling(char *str, t_all *all)
{
	if (all->ceiling != 0)
		ft_error(all, 32, "Error : Multiple ceiling inputs\n");
	all->ceiling = 1;
	while (*str == ' ')
		str++;
	...
	while ((*str >= '0' && *str <= '9') || *str == ' ')
		str++;
	if (*(str++) != ',')
		ft_error(all, 38, "Error : Incorrect ceiling color input\n");
	...
	if (all->draw->ceil_r == -1 || all->draw->ceil_g == -1 ||
		all->draw->ceil_b == -1)
		ft_error(all, 37, "Error : Wrong values for floor color\n");
	check_error(all, str);
}
...
{
	if (all->floor != 0)
		ft_error(all, 30, "Error : Multiple floor inputs\n");
	all->floor = 1;
	while (*str == ' ')
		str++;
	...
	while ((*str >= '0' && *str <= '9') || *str == ' ')
		str++;
	if (*(str++) != ',')
		ft_error(all, 36, "Error : Incorrect floor color input\n");
...
{
	if (all->res != 0)
		ft_error(all, 35, "Error : Multiple resolution inputs\n");
	all->res = 1;
	while (*str == ' ')
		str++;
	...
	while (*str >= '0' && *str <= '9')
		str++;
	if (*str != ' ')
		ft_error(all, 27, "Error : Wrong height input\n");
	while (*str == ' ')
		str++;
	...
	if (all->pos->width <= 0 || all->pos->height <= 0)
		ft_error(all, 32, "Error : Wrong resolution values\n");
